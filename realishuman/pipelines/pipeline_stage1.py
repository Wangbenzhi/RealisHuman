...
@dataclass
class StageOnePipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]


class StageOnePipeline(DiffusionPipeline):
    _optional_components = []

    def __init__(
        self,
        vae: Union[AutoencoderKL, AutoencoderKLTemporalDecoder],
        image_encoder: AutoModel,
    ...
    def check_inputs(self, pose, ref_image, ref_image_clip, ref_pose, height, width, callback_steps):
        ...
        if not isinstance(ref_pose, torch.Tensor):
            raise ValueError(f"`ref_pose` has to be of type `torch.Tensor` but is {type(ref_pose)}")
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.check_inputs(pose, ref_image, ref_image_clip, ref_pose, height, width, callback_steps)
        ...
        if ref_mean_ratio > 0.0:
            if self.image_finetune:
                latents = ref_latents * ref_mean_ratio + (1 - ref_mean_ratio) * latents
            else:
                latents = ref_latents.unsqueeze(2).repeat(
                    1, 1, video_length, 1, 1) * ref_mean_ratio + (1 - ref_mean_ratio) * latents
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ref_latent_model_input = self.scheduler.scale_model_input(ref_latents, t)
                ...
                if self.image_finetune:
                    noise_pred = self.unet(
                        sample=latent_model_input, ref_sample=ref_latent_model_input,
                        pose=pose, ref_pose=ref_pose, timestep=t,
                    ...
                    for c in context_queue:
                        ...
                        noise_pred[:, :, c] += self.unet(
                            sample=partial_latent_model_input, ref_sample=ref_latent_model_input,
                            pose=partial_pose, ref_pose=ref_pose, timestep=t,
                            encoder_hidden_states=clip_latents, drop_reference=False,
                        ).sample.to(dtype=latents_dtype)
                        counter[:, :, c] += 1
        ...
        return StageOnePipelineOutput(videos=video)
